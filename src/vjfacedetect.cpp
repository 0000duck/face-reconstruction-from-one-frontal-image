...
bool detect_all_faces(asm_shape** Shapes, int& n_shapes, const IplImage* image)
{
	...
	*Shapes = new asm_shape[pFaces->total];
	...
	for (int iFace = 0; iFace < pFaces->total; iFace++)
    {
		...
		(*Shapes)[iFace][0].x = static_cast<float>(r->x*2.0);
		(*Shapes)[iFace][0].y = static_cast<float>(r->y*2.0);
		(*Shapes)[iFace][1].x = static_cast<float> ((*Shapes)[iFace][0].x + 2.0*r->width);
		(*Shapes)[iFace][1].y = static_cast<float>((*Shapes)[iFace][0].y + 2.0*r->height);
    }
	return true;
}
